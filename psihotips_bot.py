...
def run_flask_app():
    ...
    # Use a production-ready WSGI server if this were a real web app,
    # but for Render's health check, Flask's dev server in a thread is okay.
...
if __name__ == "__main__":
    if not TOKEN:
        logger.error("Telegram TOKEN not found in environment variables! Exiting.")
        exit()
    if not ADMIN_USER_ID:
    ...
    # Setup and run Telegram bot in the main thread
    # This is required for signal handlers (e.g., for graceful shutdown)
    # to work correctly with python-telegram-bot.
    ...
    logger.info("Telegram Bot is starting polling in the main thread...")
